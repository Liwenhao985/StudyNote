...
> - JDBC是Java提供的一组独立于任何数据库管理系统的==API==。
> - Java提供接口规范，由各个数据库厂商提供接口的实现，厂商提供的实现类封装成jar文件，也就是我们俗称的==数据库驱动jar包。==
> - 学习JDBC，充分体现了面向接口编程的好处，程序员只关心标准和规范，而无需关注实现过程。
> - ![image-20250422090006761](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250422090006761.png)
...
- 但是`Statement` 接口在执行SQL语句时，会产生`SQL注入攻击问题`:
  - 当使用 `Statement` 执行动态构建的 SQL 查询时，往往需要将查询条件与 SQL 语句拼接在一起，直接将参数和SQL语句一并生成，让SQL的查询条件始终为true得到结果。
    ...
    ![image-20250412101602540](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250412101602540.png)
  
    ![image-20250412101611577](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250412101611577.png)
...
- ![image-20250422093202682](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250422093202682.png)

- ![image-20250422095112783](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250422095112783.png)

  ![image-20250412103048529](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250412103048529.png)

  ![image-20250412103002886](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250412103002886.png)
...
- 遍历结果：ResultSet可以==使用 `next()` 方法将游标移动到结果集的下一行，逐行遍历数据库查询的结果，返回值为boolean类型，true代表有下一行结果，false则代表没有。==（即next( )获取行数据，刚开始游标在字段名那一行）
- 获取单列结果：可以==通过getXxx的方法获取单列的数据==，该方法为重载方法，支持索引和列名进行获取。（即getXxx( )获取列数据）
...
```java
	//这个是导入了单元测试包之后，加个这个标记，就不用重复写main方法了
	@Test
...
- 代码实现：在sql语句后面加上“Statement.RETURN_GENERATED_KEYS”

  ​                   之后再调用resultSet = preparedStatement.getGeneratedKeys();
  ​        							  if (resultSet.next()){
  ​            								int empId = resultSet.getInt(1);
  ​            								employee.setEmpId(empId);
  ​       							    }

  ​					也就意味着==主键回显会返回单行单列的结果==

- ==注意事项：资源的释放，null时不能释放==
  ...
  - ```java
    ...
    //        Class.forName("com.mysql.cj.jdbc.Driver");
            ...
            //5.处理结果
           ResultSet resultSet = null;//放到外面，方便资源释放
            if(result>0){
                System.out.println("添加成功");
...
- 代码实现：batch
  ...
  - ```java
    ...
    //        Class.forName("com.mysql.cj.jdbc.Driver");
            ...
            //2.获取连接
            Connection connection = DriverManager.getConnection("jdbc:mysql:///test?rewriteBatchedStatements=true", "root", "123456");
    
            //3.编写SQL语句
            /*
                注意：1、必须在连接数据库的URL追加?rewriteBatchedStatements=true，允许批量操作
...
|                     主流连接池的功能对比                     |
| :----------------------------------------------------------: |
| ![image-20250413091111003](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250413091111003.png) |

|                   mock性能数据（单位：ms）                   |
| :----------------------------------------------------------: |
| ![image-20250413091200117](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250413091200117.png) |

|                   mysql性能数据 (单位：ms)                   |
| :----------------------------------------------------------: |
| ![image-20250413091224068](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250413091224068.png) |
    ...
    - ```java
      @Test
          public void testHardCodeDruid() throws SQLException {
              ...
              //2.2 非必须设置的配置
              druidDataSource.setInitialSize(10);//初始化就有10个connection对象
              druidDataSource.setMaxActive(20);//最大可以有20个connection对象，已有的不够且未达到最大个数，则会新创建
              ...
              //5.回收连接
              connection.close();//还是用close()，但此时时返回至连接池，而不是释放掉
          }
      ```
  ...
  - 软编码方式（推荐）：
    ...
    - 在项目目录下创建resources文件夹，标识该文件夹为资源目录，创建db.properties配置文件，将连接信息定义在该文件中。
      ...
      - ```properties
        ...
        password=123456
        initialSize=20
        maxActive=100//默认为8？配置了初始个数也要同步配置最大个数
        ```
...
>
> 而==这个map是通过ThreadLocal的set和get方法操作的==。==对于同一个static ThreadLocal，不同线程只能从中get，set，remove自己的变量，而不会影响其他线程的变量==。
>
...
![image-20250413105149442](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250413105149442.png)
...
>
> Java是面向对象语言，数据在Java中通常以对象的形式存在。==一张表对应一个实体类，一张表的操作对应一个DAO对象！==（表的内容对应实体类中的属性，表的操作对应DAO对象）
>
> 在Java操作数据库时，我们会==将对同一张表的增删改查操作统一维护起来，维护的这个类就是DAO层。==
>
> ==DAO层只关注对数据库的操作，供业务层Service调用，将职责划分清楚！==
>
> ![image-20250413111714111](C:\Users\lwh\AppData\Roaming\Typora\typora-user-images\image-20250413111714111.png)



#### 10.1 BaseDAO概念

> 基本上每一个数据表都应该有一个对应的DAO接口及其实现类，发现对==所有表的操作（增、删、改、查）代码重复度很高，所以可以抽取公共代码，给这些DAO的实现类可以抽取一个公共的父类==，复用增删改查的基本操作，我们称为BaseDAO。
...
public abstract class BaseDAO {
    ...
    protected int update(String sql,Object... args) throws SQLException {
        ...
        ps.close();
        //这里检查下是否开启事务，开启则暂时不关闭连接，只是业务方法关闭return!
    ...
    protected <T> ArrayList<T> query(Class<T> clazz,String sql,Object... args) throws Exception {
        ...
        if(args != null && args.length>0){
            for(int i=0; i<args.length; i++) {
                ps.setObject(i+1, args[i]);//?的编号从1开始，不是从0开始，数组的下标是从0开始
                //因为每个位置都可能不同，所以循环时要用Object这个大父类
            }
        }
